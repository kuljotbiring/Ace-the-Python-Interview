...
class LinkedList:
    ...
    def count_recursive_occurrences(self, node, val):
        if not node:
            return 0

        if node.data == val:
            return 1 + self.count_recursive_occurrences(node.next, val)
        else:
            return self.count_recursive_occurrences(node.next, val)
